...
class UniTRCAM(nn.Module):
    ...
    def _input_preprocess(self, batch_dict):
        ...
        multi_feat = patch_feat
        # multi_feat = torch.cat([voxel_feat, patch_feat], dim=0)
        multi_set_voxel_inds_list = patch_set_voxel_inds_list
        # multi_set_voxel_inds_list = [[torch.cat([set_voxel_inds_list[s][i], patch_set_voxel_inds_list[s][i]], dim=1)
        #                               for i in range(self.num_shifts[s])] for s in range(len(self.set_info))]
        multi_set_voxel_masks_list = patch_set_voxel_masks_list
        ...
        return multi_feat, None, patch_info, multi_set_voxel_inds_list, multi_set_voxel_masks_list, multi_pos_embed_list
