...
# groups
# classes 0 1 2 ...
overlap_0_7 = np.array([[0.5, 0.5, 0.5, 0.5, 0.5, 0.5,        0.5, 0.5, 0.5, 0.5, 0.5, 0.5],  # bbox
                        [0.5, 0.5, 0.5, 0.5, 0.5, 0.5,        0.5, 0.5, 0.5, 0.5, 0.5, 0.5],  # bev
                        [0.5, 0.5, 0.5, 0.5, 0.5, 0.5,        0.5, 0.5, 0.5, 0.5, 0.5, 0.5]]) # 3d
overlap_0_5 = np.array([[0.5, 0.5, 0.5,   0.5,  0.5, 0.5,     0.5,  0.5,  0.5,  0.5, 0.5, 0.5],     # bbox
                        [0.5, 0.25, 0.25, 0.5, 0.25, 0.5,     0.5, 0.25, 0.25, 0.25, 0.5, 0.5],     # bev
                        [0.5, 0.25, 0.25, 0.5, 0.25, 0.5,     0.5, 0.25, 0.25, 0.25, 0.5, 0.5]])    # 3d

CLASS_NAMES = \
    ["car","truck_head","RTG","tray_w_container","tray_wo_container","QC","lock_station","pedestrian","street_lamp"]
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    for j, curcls in enumerate(current_classes):
        i = 0 # group0-only
        overlap = min_overlaps[i, 1, j]
        result += print_str(
            (f"{class_to_name[curcls]:20} @ {overlap}\t"
                f"AP 3d: {mAP3d_R40[j, 1, i]:8.4f}, "
                f"AP bev: {mAPbev_R40[j, 1, i]:8.4f}, "
                f"AP bbox: {mAPbbox_R40[j, 1, i]:8.4f}, "
                f"AP aos: {mAPaos_R40[j, 1, i]:4.2f}"
                )) # moderate-only
    avg_mAP_bev = np.mean(mAPbev_R40[:, 1, 0])
    avg_mAP_3d = np.mean(mAP3d_R40[:, 1, 0])
    avg_mAP_bbox = np.mean(mAPbbox_R40[:, 1, 0])
    avg_mAP_aos = np.mean(mAPaos_R40[:, 1, 0])

    result += print_str((f"Average mAP 3d: {avg_mAP_3d:.4f}, "
                         f"Average mAP bev: {avg_mAP_bev:.4f}, "
                         f"Average mAP bbox: {avg_mAP_bbox:.4f}, "
                         f"Average mAP aos: {avg_mAP_aos:.2f} "
                            ))
