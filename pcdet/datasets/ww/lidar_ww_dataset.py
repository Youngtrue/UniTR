...
class LiDARWWDataset(KittiDataset):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.data_infos = self.append_data_info(kitti_infos)
    ...
    def include_kitti_data(self, mode):
        ...
        if self.logger is not None:
            self.logger.info('Total samples for WW dataset: %d' % (len(kitti_infos)))
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                annotations['name'] = np.char.replace(annotations['name'], 'tray_wo_container', 'tray_w_container')
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train', image_crops=False):
        ...
        if image_crops:
            db_info_save_path = Path(self.root_path) / ('stereo_ww_dbinfos_%s.pkl' % split)
        else:
            db_info_save_path = Path(self.root_path) / ('ww_dbinfos_%s.pkl' % split)
        ...
        for k in range(len(infos)):
            ...
            for i in range(num_obj):
                ...
                if image_crops:
                    database_img_save_path = database_save_path / 'images'
                    database_img_save_path.mkdir(parents=True, exist_ok=True)
...
def create_ww_infos(dataset_cfg, class_names, data_path, save_path, workers=4, create_infos=True, create_gt_database=True, image_crops=False):
    dataset = LiDARWWDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    train_split, val_split = 'train', 'val'

    train_filename = save_path / ('ww_infos_%s.pkl' % train_split)
    val_filename = save_path / ('ww_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'ww_infos_trainval.pkl'
    test_filename = save_path / 'ww_infos_test.pkl'
    ...
    if create_infos:
        ...
        print('ww info train file is saved to %s' % train_filename)

        dataset.set_split(val_split)
        kitti_infos_val = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
        with open(val_filename, 'wb') as f:
            pickle.dump(kitti_infos_val, f)
        print('ww info val file is saved to %s' % val_filename)

        with open(trainval_filename, 'wb') as f:
            pickle.dump(kitti_infos_train + kitti_infos_val, f)
        print('ww info trainval file is saved to %s' % trainval_filename)

        dataset.set_split('test')
        kitti_infos_test = dataset.get_infos(num_workers=workers, has_label=False, count_inside_pts=False)
        with open(test_filename, 'wb') as f:
            pickle.dump(kitti_infos_test, f)
        print('ww info test file is saved to %s' % test_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./configs/lidar/dataset_configs/ww_dataset_debug.yaml')
    parser.add_argument('--image_crops', action='store_true')
    parser.add_argument('--workers', type=int, default=4)
    args = parser.parse_args()

    if args.command == 'create_ww_infos':
        dataset_cfg = EasyDict(yaml.load(open(args.cfg), Loader=yaml.FullLoader))
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        create_ww_infos(
            dataset_cfg=dataset_cfg,
            class_names=["car","truck_head","RTG","tray_w_container","QC","lock_station","street_lamp"],
            data_path = ROOT_DIR / dataset_cfg.DATA_PATH,
            save_path = ROOT_DIR / dataset_cfg.DATA_PATH,
            # data_path=ROOT_DIR / 'data' / 'ww',
            # save_path=ROOT_DIR / 'data' / 'ww',
            create_infos=True,
